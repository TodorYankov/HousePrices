...
# Инсталирайте XGBoost ако нямате
# pip install xgboost

import xgboost as xgb

print("=" * 80)
print("HOUSE PRICES - ПОДОБРЕН МОДЕЛ (XGBoost + Feature Engineering)")
print("=" * 80)
...
def fix_missing_values(df):
    ...
    # Categorical features - запълваме с 'None' или най-често срещаното
...
def add_features(df):
    ...
    # Обща площ (жилищна + мазе)
    data['TotalSF'] = data['TotalBsmtSF'] + data['GrLivArea']

    # Обща баня (пълна + половина)
    data['TotalBath'] = data['FullBath'] + 0.5 * data['HalfBath'] + data['BsmtFullBath'] + 0.5 * data['BsmtHalfBath']

    # Общо качество на къщата
...
for col in cat_cols:
    if col in train.columns and col != 'SalePrice':
        le = LabelEncoder()
        # Комбинираме train и test за еднакво кодиране
...
print("\n7. Обучение на финален XGBoost модел...")
...
submission.to_csv('submission_house_prices_xgboost.csv', index=False)

print(f"\n✅ Файл: submission_house_prices_xgboost.csv")
...
print("\n" + "=" * 80)
print("📈 СРАВНЕНИЕ С ПРЕДИШНИЯ МОДЕЛ:")
print("=" * 80)
print(f"   Първи модел (RF, 11 features): 0.15793")
print(f"   XGBoost ({len(feature_cols)} features): {np.mean(cv_scores):.4f} (CV)")
print("=" * 80)

print("\n🚀 ГОТОВО ЗА КАЧВАНЕ В KAGGLE!")
print("📁 Файл: submission_house_prices_xgboost.csv")
