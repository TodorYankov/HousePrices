# house_prices_improved_v_10.py
# Финален модел с Lasso feature selection (63 от 110 признака)
# Базиран на v_06 (5 модела + Ridge alpha=0.02)
...
print("=" * 80)
print("🏠 HOUSE PRICES - ФИНАЛЕН МОДЕЛ V_10")
print("   5 базови модела + Ridge(alpha=0.02) + Lasso Feature Selection")
...
# 6. FEATURE SELECTION С LASSO
print("\n6. Избор на признаци с LassoCV...")
...
# 7. ОБУЧЕНИЕ НА STACKING МОДЕЛ (5 базови + Ridge meta)
print("\n7. Обучение на Stacking модел (5-fold CV)...")
...
# OOF прогнози
oof_rf = np.zeros(len(X_selected))
oof_xgb = np.zeros(len(X_selected))
oof_lgb = np.zeros(len(X_selected))
oof_gb = np.zeros(len(X_selected))
oof_ridge = np.zeros(len(X_selected))

test_preds_rf = np.zeros((len(X_test_selected), 5))
test_preds_xgb = np.zeros((len(X_test_selected), 5))
test_preds_lgb = np.zeros((len(X_test_selected), 5))
test_preds_gb = np.zeros((len(X_test_selected), 5))
test_preds_ridge = np.zeros((len(X_test_selected), 5))
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_selected)):
    ...
    test_preds_lgb[:, fold] = lg.predict(X_test_selected)

    gb = GradientBoostingRegressor(**gb_params)
    gb.fit(X_tr, y_tr)
    oof_gb[val_idx] = gb.predict(X_val)
    test_preds_gb[:, fold] = gb.predict(X_test_selected)

    ridge = Ridge(**ridge_params)
    ridge.fit(X_tr, y_tr)
    oof_ridge[val_idx] = ridge.predict(X_val)
    test_preds_ridge[:, fold] = ridge.predict(X_test_selected)

# 8. МЕТА-МОДЕЛ
print("\n8. Обучение на мета-модел (Ridge alpha=0.02)...")
...
# 9. SUBMISSION
print("\n9. Създаване на submission файл...")
submission = pd.DataFrame({
    'Id': test['Id'],
    'SalePrice': final_predictions
})
submission.to_csv('submission_house_prices_improved_v_10.csv', index=False)

print("\n" + "=" * 80)
print("✅ МОДЕЛ V_10 ЗАВЪРШЕН!")
print("=" * 80)
...
print(f"📁 Файл: submission_house_prices_improved_v_10.csv")
