# test_ransac.py
# Тества дали замяната на Ridge с RANSAC подобрява резултата
...
print("=" * 80)
print("🔬 ТЕСТ: RANSAC вместо Ridge в ансамбъла")
print("=" * 80)
...
# ============================================================================
print("\n📊 Сравнение на Ridge vs RANSAC като базов модел (5-fold CV)")
...
rmsle_ridge, _ = evaluate(models_ridge, X_scaled, y, kf)
rmsle_ransac, _ = evaluate(models_ransac, X_scaled, y, kf)

print("\n" + "=" * 80)
print("📊 РЕЗУЛТАТИ:")
print(f"   С Ridge (v_06): RMSLE = {rmsle_ridge:.5f}")
print(f"   С RANSAC:        RMSLE = {rmsle_ransac:.5f}")
improvement = rmsle_ridge - rmsle_ransac
if improvement > 0:
    print(f"\n✅ RANSAC подобрява резултата с {improvement:.5f}")
    print("💡 Препоръка: Използвайте RANSAC вместо Ridge във финалния модел.")
else:
    print(f"\n❌ RANSAC не подобрява резултата (разлика {improvement:.5f})")
    print("💡 Препоръка: Останете с Ridge (v_06).")
print("=" * 80)
