# test_ransac_addition.py
# Тества дали добавянето на RANSAC като 6-ти базов модел подобрява резултата
...
print("=" * 80)
print("🔬 ТЕСТ: ДОБАВЯНЕ НА RANSAC КЪМ 5-ТЕ БАЗОВИ МОДЕЛА")
print("=" * 80)
...
# ============================================================================
# 3. ОЦЕНКА НА ДВАТА ВАРИАНТА
# ============================================================================
print("\n📊 Сравнение: 5 модела vs 5 модела + RANSAC (5-fold CV)")

kf = KFold(n_splits=5, shuffle=True, random_state=42)

def evaluate(models, X_data, y_data, kf):
...
rmsle_5, _ = evaluate(models_5, X_scaled, y, kf)
rmsle_6, _ = evaluate(models_6, X_scaled, y, kf)

print("\n" + "=" * 80)
print("📊 РЕЗУЛТАТИ:")
print(f"   5 модела (v_06):        RMSLE = {rmsle_5:.5f}")
print(f"   5 модела + RANSAC:      RMSLE = {rmsle_6:.5f}")
improvement = rmsle_5 - rmsle_6
if improvement > 0:
    print(f"\n✅ Добавянето на RANSAC подобрява резултата с {improvement:.5f}")
    print("💡 Препоръка: Направете submission с 6-те модела (добавете RANSAC).")
else:
    print(f"\n❌ Добавянето на RANSAC влошава резултата с {-improvement:.5f}")
